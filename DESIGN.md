...
New `.sort-select` sits next to `.search-field` in `.toolbar`. Plain `<select>`, not a custom dropdown — this is a two-option control ("Newest first" / "Most viewed"), a native select is the right amount of UI for that, no need for a styled listbox.

**Shape and focus style corrected (2026-08-24, screenshot-caught).** Originally matched `.search-field`'s pill shape (`border-radius: 999px`) with the same near-black `var(--focus)` outline used everywhere else on the page — read as an unfamiliar/non-standard control once actually clicked: a pill-shaped closed box opening into the browser's own rectangular native option list looked mismatched, and the 2px `var(--focus)` ring on open read as a thick dark border. Fixed by studying `My-Investment-Port`'s own `<select>` pattern (`src/components/ui/form-styles.css`): a plain `10px` rounded rect (not a pill — matches the shape of the native dropdown menu it opens into), `appearance: none` + a custom SVG chevron replacing the native arrow (the native open *menu* itself is left alone — that part isn't stylable via CSS in any browser, don't try). Focus ring switched from `var(--focus)` to `var(--teal)` at `1px` — lighter and already documented in the Colors table above as the token for 3:1 UI-component contrast (not text), the exact WCAG tier a focus indicator needs, so this isn't a special case, it's using the correct existing token. Don't reuse `var(--focus)` on future dropdown-style controls — that token's near-black weight reads as "thick border" specifically on a small pill/rounded control; it's calibrated for larger surfaces like the search field and chips.
